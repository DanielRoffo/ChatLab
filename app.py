...
def register():
    ...
    if request.method == "POST":
        ...
        msg = Message(subject = 'App Security Code', sender = '**********@gmail.com', recipients =[email])
        msg.body = str(otp)
        mail.send(msg)   
...
def changePassword():
    ...
    if request.method == "POST":
        ...
        # Toma los datos del nuevo usuario y manda un email al usuario.
        msg = Message(subject = 'App Security Code', sender = '*************@gmail.com', recipients = [session.get("user_email")])
        msg.body = str(otp)
        mail.send(msg)   
        cur.execute("DELETE FROM preregister")
